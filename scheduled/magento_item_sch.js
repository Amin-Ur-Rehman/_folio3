    ...
    return {
        ...
        upsert: function (data, id) {
            try {
                ...
                nlapiLogExecution('DEBUG', 'InventorySyncScript.upsert', id);
            } catch (e) {
                nlapiLogExecution('ERROR', 'InventorySyncScript.upsert', e.toString());
            }
            return id;
        },
        lookup: function (filters) {
            ...
            try {
                ...
                nlapiLogExecution('ERROR', 'InventorySyncScript.lookup', e.toString());
            }
            return result;
        },
        updateStatus: function (name, lastRunDateTime) {
            var fils = [];
            fils.push(new nlobjSearchFilter(this.FieldName.Name, null, 'is', name));
