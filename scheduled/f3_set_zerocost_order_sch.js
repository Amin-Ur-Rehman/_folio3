...
/**
 * SetZeroCostOrder class that has the actual functionality of adding handling cost in synced magento orders
 * All business logic will be encapsulated in this class.
 */
var SetZeroCostOrder = (function () {
    return {
        ...
        getRecords: function (lastId) {
            ...
            records = nlapiSearchRecord(null, 82);
        ...
        processRecords: function (records) {
            ...
            for (var i = 0; i < count; i++) {
                ...
                try {
                    ...
                    var incrementId = record.getValue('custbody_magentoid', null, 'GROUP');
                    id = record.getValue('internalid', null, 'GROUP');

                    nlapiLogExecution('DEBUG', 'Order ID ' + id, 'Magento Order Id ' + incrementId);

                    var baseUrl = 'https://www.purestcolloids.com/cart';
                    var url = baseUrl + '/f3-set-cost-to-zero-in-order.php?incrementid=' + incrementId + '&method=setitemcost';
        ...
        markRecords: function (id) {
            ...
            try {
                nlapiSubmitField('salesorder', id, 'custbody_zero_amt', 'T');
            } catch (e) {
 ...
 * @returns {Void}
 */
function SetZeroCostOrderScheduled(type) {
    return SetZeroCostOrder.scheduled(type);
}